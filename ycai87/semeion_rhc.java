...
public class semeion_rhc {
    private static Instance[] instances = initializeInstances();
    private static Instance[] train_set = Arrays.copyOfRange(instances, 0, 1274);
    private static Instance[] test_set = Arrays.copyOfRange(instances, 1274, 1593);

    private static DataSet set = new DataSet(train_set);

    private static int inputLayer = 256, hiddenLayer= 100, outputLayer = 10;
        ...
        for (int trainingIterations : iterations) {
            ...
            for (int i = 0; i < oa.length; i++) {
                ...
                for (int j = 0; j < train_set.length; j++) {
                    ...
                    L =  train_set[j].getLabel().toString().split(", ");
                    int l = 0;
                    for(String s : L){
                        if(Double.parseDouble(s) == new Double(1))
                            break;
                        l++;
                    }
                    O =  networks[i].getOutputValues().toString().split(", ");
                    int o = 0;
                    int idx = 0;
                    ...
                    actual =  l;//Double.parseDouble(train_set[j].getLabel().toString());
                    predicted = o;//Double.parseDouble(networks[i].getOutputValues().toString());

                    //System.out.println("actual is " + actual);
                    //System.out.println("predicted is " + predicted);

                    double trash = (l == o) ? correct++ : incorrect++;
                ...
                write_output_to_file("Optimization_Results", "semeion_results_rhc.csv", final_result, true);
                ...
                for (int j = 0; j < test_set.length; j++) {
                    ...
                    L =  test_set[j].getLabel().toString().split(", ");
                    int l = 0;
                    for(String s : L){
                        if(Double.parseDouble(s) == new Double(1))
                            break;
                    ...
                    actual = l;//Double.parseDouble(test_set[j].getLabel().toString());
                    predicted = o;//Double.parseDouble(networks[i].getOutputValues().toString());
    ...
    private static Instance[] initializeInstances() {
        ...
        double[][][] attributes = new double[1593][][];

        try {
            BufferedReader br = new BufferedReader(new FileReader(new File("src/opt/ycai87/semeion.data")));
            ...
            for(int i = 0; i < attributes.length; i++) {
                ...
                attributes[i][0] = new double[256]; // 16 attributes
                attributes[i][1] = new double[10]; // classification
